...
** The sales database is built with MySQL, backed by a cinder persistent volume from OpenStack.
...
.  Log into the OpenStack dahsboard and start both OpenShift instances:

* Username: admin
* Password: r3dh4t1!
* Click on Project and Instances
* For each instance click "Edit Instance", and "Hard Reboot Instance".
* Once running, click on openshift-master.example.com to view it's details.
* Confirm the Cinder volume used for persistent storage is "datagrid" on /dev/vdc
...
This terminal will be used later in the demonstration

== Review the Environment

Red Hat Cloud Suite enables end users to create, deploy and update micro-services based applications.

OpenShift Enterprise Console:  `https://workstation-<GUID>.rhpds.opentlc.com:8443/console`

.  Once the OpenShift environment is up and running (this may take several minutes) login to the OSE console:
...
The environment is configured with persistent storage that maps a Cinder volume in OpenStack to a persistent volume in OpenShift.  A persistent claim is then mapped to the volume and provides storage to the sales-db pod.
This can be illustrated to viewers of the demonstration from the terminal session that was established to workstation-<GUID>.rhpds.opentlc.com in the getting started section.

.  On the OpenShift all-in-one instance (openshift-master.example.com) review the persistent volume and claim. From the workstation-<GUID> system ssh to the openshift master perform the following steps.
* ssh -i openshift.pem cloud-user@192.168.1.250
* sudo -i
* oc login (username: admin password: r3dh4t1!)
* From the cli: oc get pv and oc get pvc
...
By utilizing OpenShift it's possible to streamline the deployment of new code changes through a Continuous Deployment (CD) pipeline.  For example, if a change is  made in the product description or if new products are added, this code change can easily be committed and then updated via OpenShift, continuous build and deployment capabilities.
...
=== Fork Repository and Edit the Code
...
If you have not done so already: from the Git web interface, click *Fork* in the
 upper right corner of the GitHub web UI to fork the Git repository
  ...
  Edit the file 'https://github.com/GitHubUsername/OpenShift3-MSA/blob/master/Presentation/src/main/webapp/products.jsp' where GitHubUsername is your github username.
  Make a change to the file such as changing the word "Availability" on line 28 to "In-Stock". You can do this inline on github or using your favorite editor and then merging.

=== Change Build location and Build

. Log into the ose-master node.
* ssh -i .ssh/id_rsa <RHPDS_username>@workstation-<GUID>.rhpds.opentlc.com
* sudo su - (password: r3dh4t1!)
* ssh -i openshift.pem cloud-user@192.168.1.250
* sudo su - (password: r3dh4t1!)
...
----
 ...
 . Verify changes
 * Refresh the web UI at `http://workstation-<GUID>.rhpds.opentlc.com/` and verify "Available" has been changed to "In Stock"
 ...
 * Click Configuration > Refresh items and relationships
 ...
 * Click Containers.  Notice the sti-build items and associated pods.  This provides the source to image CI capabilities demonstrated previously
...
The JBoss EPA application provides order functionality in the web UI for products listed.  Create a test user and purchase several products.

  . Create a test user
  * Navigate to the EAP web UI: `http://workstation-<GUID>.rhpds.opentlc.com/`
